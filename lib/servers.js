...
var ServerInstance = utils.ServerInstance;
...
Servers.prototype.httpServer = function(options) {
	...
	server.on('request', options.request).on('close', function() {
		self.emit('close', ServerInstance);
	});
	//add a web-socket if needed.
	if(options.hasSocket) {
		this.ioServer(server)
	}
	server.listen(options.port, options.host, function() {

		self.emit('open', ServerInstance);
	});
...
Servers.prototype.tcpServer = function(options) {
	...
	options = mixin(options, {
		...
		socket : function(socket) {
			...
			self.sockets[socket.id] = socket.on('close', function() { delete
...
Servers.prototype.ioServer = function(options) {
	...
	server.on('connection', options.socket).on('close', options.close)
	server.listen(options.server || options.port)
